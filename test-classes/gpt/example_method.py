...
from .replay_gan import ClientGANReplay, GANReplayConfig
...
def build_pretrained_resnet18_backbone() -> tuple[nn.Module, int]:
    ...
    m.fc = nn.Identity()  # feature extractor
    return m, feat_dim
...
class ExampleFedAvgWithGANReplay(BaseFCILAlgorithm):
    ...
    def __init__(
        ...
        backbone, feat_dim = build_pretrained_resnet18_backbone()
        ...
        server_replay = ClientGANReplay(gan_cfg, device=device)
        ...
        for cid in range(num_clients):
            ...
            c_replay = ClientGANReplay(gan_cfg, device=device)
